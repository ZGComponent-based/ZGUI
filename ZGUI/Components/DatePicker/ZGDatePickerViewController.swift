...
public class ZGDatePickerViewController: UIViewController {

    
    public var canPickBeforeTime:Bool = true

    public var backDate: ((Date) -> Void)?
    ///获取当前日期
    ...
    private func drawMyView(){
        ...
        cancel.setTitleColor(UIColor.colorOfHexText("#2D5BFF"), for: .normal)
        sure.setTitleColor(UIColor.colorOfHexText("#2D5BFF"), for: .normal)
    ...
    @objc func onClickCancel() {
        ...
        if let d = self.touchHandle {
            d()
        }
    }
    @objc func onClickSure() {
        let dateString = String(format: "%02ld-%02ld-%02ld", self.picker.selectedRow(inComponent: 0) + (2018), self.picker.selectedRow(inComponent: 1) + 1, self.picker.selectedRow(inComponent: 2) + 1)
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "YYYY-MM-dd"
        if canPickBeforeTime{
            /// 直接回调显示
            ...
            /*** 如果需求需要不能选择已经过去的日期*/
                    ...
                    if let window = UIApplication.shared.delegate?.window {
                        ZGUIToastView.showInView(window!, text: "请选择今天和今天之后的日期")
                    }
...
extension ZGDatePickerViewController:UIPickerViewDelegate,UIPickerViewDataSource {
    ...
    public func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        if component == 0 {
            ...
            let year: Int = pickerView.selectedRow(inComponent: 0) + 2018
...
//MARK: - 转场动画delegate
extension ZGDatePickerViewController:UIViewControllerTransitioningDelegate{
    public func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        let animated = ZGDatePickerPresentAnimated(type: .present)
        return animated
    }
    public func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        let animated = ZGDatePickerPresentAnimated(type: .dismiss)
        return animated
    }
}
